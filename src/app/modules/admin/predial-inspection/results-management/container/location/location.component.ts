...
} from '@fuse/services/splash-screen';
...
export class LocationComponent implements OnInit, OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (this.ticket && this.ubicacion) {
            this.codTipoTicket = this.ticket.codTipoTicket
                ...
                if (this.dataPoint.type === TypePoint.PUNTO_IMAGEN) {
                    if (
                        [
                            ...
                            TypeGap.PUNTOS_LOTE_SIN_PREDIO,
                            TypeGap.PUNTO_IMAGEN,
                        ].includes(this.ticket.codTipoTicket)
                    ) {
    ...
    ngOnInit(): void {
        this.checkTicketService.resolverTicket$.subscribe((option: string) => {
            console.log(option);
            console.log('this.datta', this.data );
            if (option === 'confirmar') {
                    this._registroTitularidadService
                        .update(this.data.registrosTitularidad.codTit, { status: 6 })
                        .subscribe((r2) => {
                            this.resolverPredio();
                        });
            } else if (option === 'subvaluar') {

                const dialogRef2 = this._messageProviderService.showModal(
                    ...
                    dialogRef2
                        ...
                        .toPromise()
                        .then((text) => {
                            this.generarNotificacionSubvaluado(
                                this.data.registrosTitularidad,
                                text
                            );
                        });
            }else {
    ...
    onClickAprTicket(): void {
        ...
        this.dialogRef.afterClosed().subscribe((result: any) => {
            ...
            if (result && result === 'confirmed') {
                ...
                this.ubicacion.status =
                    TicketStatus.RESUELTO_GESTION_RESULTADOS;
                this._ubicacionService
                    .update(this.ubicacion.codUbicacion, {
                        status: TicketStatus.RESUELTO_GESTION_RESULTADOS,
                    })
                    .subscribe((res) => {
                        this.updateLocation(
                            this.ubicacion,
                            this.ticket.nroNotificacion
                        );
                        if (
                            this.ticket.codTipoTicket ===
                            TypeGap.MANZANA_SIN_LOTES
                        ) {
    ...
    onClickObsTicket(): void {
        this.dialogRef = this._messageProviderService.showModal(
            ModalComponent,
            { width: 600 }
        );
        this.dialogRef.afterClosed().subscribe((result: any) => {
            if (result && result.option) {
                ...
                this.ubicacion.status =
                    TicketStatus.OBSERVADO_GESTION_RESULTADOS;
                ...
                this._ubicacionService
                    .updateObs(this.ubicacion.codUbicacion, data)
                    .subscribe((res) => {
                        if (
                            this.ticket.codTipoTicket ===
                            TypeGap.MANZANA_SIN_LOTES
                        ...
                        './land-inspection/results-management',
                            ...
                            this._registroTitularidadService
                                .update(
                                    this.ubicacion.registrosTitularidad[0]
                                        .codTit,
                                    {
                                        status: TicketStatus.OBSERVADO_GESTION_RESULTADOS,
                                    }
                                ...
                                .subscribe((r2) => {
                                    this._ticketService
                                        .update(this.ticket.codTicket, {
                                            codEstTrabajoTicket:
                                                TicketStatus.OBSERVADO_GESTION_RESULTADOS,
                                        })
                                        .subscribe((r) => {
                                            this.actualizarCFTicket(
                                                this.ticket.codTicket,
                                                TicketStatus.OBSERVADO_GESTION_RESULTADOS
                                            );
                                            this.updateLocation(this.ubicacion);
                                            this._router.navigate([
    ...
    actualizarCFTicket(codTicket: string, estado: any): void {
        ...
        this._cfTicketService
            .getTicket({ COD_TICKET: codTicket, ESTADO_V: 1 })
            .then((responseJson) => {
                ...
                if (responseJson && responseJson.features) {
                    ...
                    this._cfTicketService
                        .updateTicket({ OBJECTID: id, ESTADO: estado })
                        .then((response) => {
                            console.log(
                                'return actualizar CF Ticket',
                                response
                            );
                        });
                }
            ...
            this.dialogRef
                ...
                .then((option) => {
                    if (option === 'confirmed') {
                        ...
                        puntoImagen.TIP_DOC =
                            this.predio.predioContribuyente[0].contribuyente.tipDoc;
                        puntoImagen.AP_MAT =
                            this.predio.predioContribuyente[0].contribuyente.apMat;
                        puntoImagen.AP_PAT =
                            this.predio.predioContribuyente[0].contribuyente.apPat;
                        puntoImagen.DOC_IDEN =
                            this.predio.predioContribuyente[0].contribuyente.docIden;
                        puntoImagen.DIR_FISCAL =
                            this.predio.predioContribuyente[0].contribuyente.dirFiscal;
                        ...
                        const index =
                            this.ubicacion.registrosTitularidad.findIndex(
                                (r) => data.id === r.id
                            );
                        this.ubicacion.registrosTitularidad[0] = data;
                        this._cfpuntoImagenService
                            .getMaxSecuen(this.ubicacion.ubigeo)
                            .then((res) => {
                                const secuen = res + 1;
                                ...
                                this._ubicacionService
                                    .update(this.ubicacion.id, this.ubicacion)
                                    .subscribe((r) => {
                                        this._cfpuntoImagenService
                                                ...
                                                if (result) {
                                                    this._confirmationService
                                                        ...
                                                        .then((e) => {
                                                            const requestArray =
                                                                [
                                                                    this._predioPadronService.updateLand(
                                                                        data
                                                                            .predioPadron
                                                                            .id,
                                                                        {
                                                            ...
                                                            forkJoin(
                                                                ...
                                                                (results) => {
                                                                    this.resolverPredio();
                                                                    this.getStatusButton();
                                                                    this._resultsService.setResetMap(
                                                                        2
                                                                    );
                                                                    this._resultsService.setEstado(
                                                                        Estado.LEER
                                                                    );
                                                                }
                                                            );
                    ...
                    if (option === 'confirmed') {
                        this._cfpredioService
                            ...
                            .then((res) => {
                                ...
                                predio.COORD_X =
                                    this.dataPoint.point['COORD_X'];
                                predio.COORD_Y =
                                    this.dataPoint.point['COORD_Y'];
                                if (res && res.COD_CPU) {
                                    ...
                                    this._registroTitularidadService
                                        .update(data.codTit, { status: 6 })
                                        .subscribe((r) => {
                                            const query = {
                                                // eslint-disable-next-line @typescript-eslint/naming-convention
                                                COD_PRE:
                                                    data.predioInspeccion
                                                        .codPre,
                                                // eslint-disable-next-line @typescript-eslint/naming-convention
                                                UBIGEO: data.predioInspeccion
                                                    .ubigeo,
                                            };
                                            const params =
                                                CommonUtils.deleteKeysNullInObject(
                                                    query
                                                );
                                            this._cfpredioService
                                                .getPredios(params)
                                                                        ...
                                                                        .then(
                                                                            (
                                                                                ...
                                                                                if (
                                                                                    ...
                                                                                    const requestArray =
                                                                                        [
                                                                                            this._predioInspeccionService.update(
                                                                                                data
                                                                                                    .predioInspeccion
                                                                                                    .id,
                                                                                                {
                    ...
                    if (option === 'confirmed') {
                        this._cfpredioService
                            ...
                            .then((res) => {
                                ...
                                if (res && res.COD_CPU) {
                                    predio.ID_LOTE =
                                        this.dataPoint.point.ID_LOTE;
                                    predio.ID_LOTE_P =
                                        this.dataPoint.point.ID_LOTE_P;
                                    predio.UBIGEO =
                                        data.predioInspeccion.ubigeo;
                                    ...
                                    predio.ESTADO = 5;
                                    predio.RAN_CPU =
                                        this.dataPoint.point.RAN_CPU;
                                    predio.COD_UI = res.COD_CPU
                                        ? parseInt(
                                            res.COD_CPU.split('-')[1],
                                            10
                                        )
                                        : null;
                                    predio.COD_VER = res.COD_CPU
                                        ? parseInt(
                                            res.COD_CPU.split('-')[2],
                                            10
                                        )
        ...
        if (!this.dataPoint) {
            ...
            this.dialogRef
                ...
                .then((option) => {
                    if (option === 'confirmed') {
                        this._cfloteService
                            .generateRANCPU(data.predioInspeccion.ubigeo)
                            .then((rancpu) => {
    ...
    getStatusButton(): void {
        const cantResueltos = this.ubicacion.registrosTitularidad.filter(
            r => r.status === 6
        ).length;
    ...
    onClickPosition(event: any): void {
        this._resultsService.setUbicacionData({
    ...
    eventIniciarItem(registro: IRegistroTitularidad): void {
        ...
        if (
            this.ubicacion.status === 0 &&
            registro.codTipoTit === 1 &&
            this.ticket.codTipoTicket !== TypeGap.MANZANA_SIN_LOTES
        ) {
            this.resetMap(2);
            ...
            .then((option) => {
                if (option === 'confirmed') {
                    ...
                    dialogRef2
                        ...
                        .toPromise()
                        .then((text2) => {
                            const contribuyente = r.suministro?.contribuyente
                                ? `${r.suministro?.contribuyente?.nombre} ${r.suministro?.contribuyente?.apPat} ${r.suministro?.contribuyente?.apMat}`
                                : r.predioInspeccion?.predioContribuyente[0]
                                    ?.contribuyente
                                ? `${r.predioInspeccion?.predioContribuyente[0]?.contribuyente?.nombre} ${r.predioInspeccion?.predioContribuyente[0]?.contribuyente?.apPat} ${r.predioInspeccion?.predioContribuyente[0]?.contribuyente?.apMat}`
                                : r.predioPadron?.predioContribuyente[0]
                                    ?.contribuyente
                                ? `${r.predioPadron?.predioContribuyente[0]?.contribuyente?.nombre} ${r.predioPadron?.predioContribuyente[0]?.contribuyente?.apPat} ${r.predioPadron?.predioContribuyente[0]?.contribuyente?.apMat}`
                                : '';
                            ...
                            this._registroTitularidadService
                                ...
                                .subscribe((response) => {
                                    ...
                                    saveAs(
                                        blob,
                                        'CARTA DE INVITACION INSCRIPCION.pdf'
                                    );
            ...
            .then((option) => {
                if (option === 'confirmed') {
                    ...
                    dialogRef2
                        ...
                        .toPromise()
                        .then((text2) => {
                            this._resultsService.setGenerarNotificacion(
                                this.ubicacion
                            );
                            ...
                            setTimeout(async () => {
                                ...
                                const screenshot =
                                    await this.view.takeScreenshot({
                                        format: 'jpg',
                                        quality: 100,
                                    });

                                const contribuyente = r.suministro
                                    ?.contribuyente
                                    ? `${r.suministro?.contribuyente?.nombre} ${r.suministro?.contribuyente?.apPat} ${r.suministro?.contribuyente?.apMat}`
                                    : r.predioInspeccion?.predioContribuyente[0]
                                        ?.contribuyente
    ...
    generarPdf(data: any): void {
        ...
        return pdfMake
            .createPdf(pdf)
            .download('Carta DE Invitacion de Inscripcion.pdf');
    }

    updateLocation(ubicacion: IUbicacion, nroNoticacion: number = null): void {
        this.eventUpdateLocation.emit({
            codUbicacion: ubicacion.codUbicacion,
            nroNoticacion: nroNoticacion,
        });
    }
    ...
    previsualizacion(registroTitularidad: IRegistroTitularidad): void {
        ...
        this.data = {
            registrosTitularidad: registroTitularidad,
            fotos: this.ubicacion.fotos,
            ticket: this.ticket,
            ubicacion: this.ubicacion,
        };

        this.checkTicketService.checkTicket$.next(true);
        this.checkTicketService.dataTicket$.next(this.data);
    ...
    */
    ...
    // DESDE AQUI
        ...
        // const dialogRef = this._messageProviderService.showModal(
        ...
        //         if (option === 'confirmar') {
        //             this._registroTitularidadService
        //                 .update(registroTitularidad.codTit, { status: 6 })
        //                 .subscribe((r2) => {
        //                     this.resolverPredio();
        //                 });
        //         } else if (option === 'subvaluar') {
    ...
    generarNotificacionSubvaluado(data: IRegistroTitularidad, text): void {
        ...
        this._registroTitularidadService
            .generarNotificacionSubvaluado(payload)
            .subscribe((response) => {
                const blob = new Blob([response], { type: 'application/pdf' });
                saveAs(blob, 'CARTA DE INVITACION INSCRIPCION.pdf');

                this._confirmationService
                    .success('Notificar', 'Notificacion generada')
                    .afterClosed()
                    .toPromise()
                    .then((res) => {
                        this._registroTitularidadService
    ...
    generarPdfPredioSubvaluadoOld(data: IRegistroTitularidad, text): any {
        ...
        autoTable(doc, {
            ...
            body: [
                ...
                [
                    {
                        content: `CARTA DE ACTUALIZACION DE VALOR Nº${
                            this.ticket.codTicket
                        }-${
                            this.ticket.nroNotificacion
                                ? this.ticket.nroNotificacion + 1
                                : 1
                        }-2023`,
                        styles: { halign: 'center' },
                    },
                ...
                [
                    {
                        content: `Municipalidad ${
                            this.distrito?.name
                        }, Mes ${moment().format('MMMM')} ${moment().format(
                            'YYYY'
                        )}`,
                        styles: { halign: 'right' },
                    },
        ...
        autoTable(doc, {
            ...
            body: [
                [
                    {
                        content:
                            'INFORMACIÓN RESULTANTE DE LA INSPECCION PREDIAL',
                        styles: { halign: 'justify' },
                        colSpan: 8,
                    },
                ],
...
*/
        ...
        this.ticket.nroNotificacion = this.ticket.nroNotificacion
            ? this.ticket.nroNotificacion + 1
            : 1;
